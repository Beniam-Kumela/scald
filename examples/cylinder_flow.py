...
solid = create_obstacle_mask(nx, ny, np.array(cylinder_centers_x), np.array(cylinder_centers_y), n_cylinders, "cylinder", width, solid)

# Reynolds Number Calculation
viscosity = (tau_f - 0.5) / 3
reynolds_number = (wind_speed * width * 2) / viscosity

print(f"Starting flow past a cylinder simulation.")
fig, ax = plt.subplots()
...
def update(frame):
    ...
    ax.set_title(f"Flow past cylinder, Re = {reynolds_number:.2e}, Time = {frame}")
    return [im]

ani = FuncAnimation(fig, update, frames = tqdm(range(iterations)), blit = True)
ani.save(filename=f"cylinder_flow_re_{reynolds_number:.2e}.mp4", writer = "ffmpeg", fps=60)
print(f"Simulation completed successfully and saved as cylinder_flow_re_{reynolds_number:.2e}.mp4 in the main directory.")
del ani